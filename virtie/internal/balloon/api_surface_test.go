...
func TestBalloonPackageDoesNotReExportConfigTypes(t *testing.T) {
	...
	for name := range exportedDecls(t) {
		if _, ok := banned[name]; ok {
			t.Fatalf("balloon should not expose config owner name %s", name)
		}
	}
}
...
func exportedDecls(t *testing.T) map[string]struct{} {
	...
	pkg := files["balloon"]
	if pkg == nil {
		t.Fatal("package balloon not found")
	}
