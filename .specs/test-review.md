...
Review and rewrite the Agentspace test surface so broad checks cover consumer workflows end to end while narrow tests stay focused on integration contracts.
...
- Keep tightly scoped tests for generated manifest and launcher contracts.
...
- changing public `mkSandbox` or `mkLaunch` behavior
...
- [x] Run final Agentspace Nix validation.
