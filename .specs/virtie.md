...
- [x] Manifest-provided `run_tunnels[]` commands run before QEMU, are watched for Unix socket readiness under `state_dir/tunnels`, and are stopped during teardown.
...
- [x] Add `run_tunnels[]` manifest support for tunnel-dir-relative Unix socket producer commands.
...
- Current public manifest contract:
  ...
  - Optional `run_tunnels[]` entries contain `exec` and a clean relative `socket` path under `state_dir/tunnels`; `virtie` injects the resolved socket path as `VIRTIE_TUNNEL_SOCKET` and starts the command in the socket directory.
...
- Runtime socket policy:
  ...
  - `virtie` injects `VIRTIE_TUNNEL_SOCKET` for each tunnel command and resolves those paths under manifest `state_dir/tunnels`, not the runtime socket directory.
