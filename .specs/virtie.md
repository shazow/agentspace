...
- Current public manifest contract:
  ...
  - Manifest exec arrays render Go `text/template` values per argv element. Commands that `virtie` starts directly also receive the same context through uppercase environment variables, and the host process environment is available under `.Env`. `qemu.fwd_tunnel_exec` is rendered per guest forward but does not receive injected environment variables because QEMU starts those commands directly.
- Runtime assumptions:
