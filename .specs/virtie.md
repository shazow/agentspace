...
- Implementation notes:
  ...
  - `virtie launch` writes final shutdown stats after teardown and cleanup complete; non-SSH launches omit the SSH-session interval. Verbose package logs use `github.com/lmittmann/tint` records on stderr, with package identity carried as an attribute such as `package=manager`; `-v` logs host command starts and `-vv` also logs host daemon/helper stdout and stderr lines at DEBUG. Active SSH session output is not duplicated into DEBUG logs.
