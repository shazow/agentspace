...
- [x] The manifest emitted from `sandbox-qemu.nix` carries the current `virtie` v2 inputs: working dir, lock path, ssh argv/user, host facts, kernel paths, volumes, mounts with managed `virtiofsd` commands, and opt-in `9p` shares.
...
- [x] Move QEMU policy and final argv construction out of Nix and into `virtie`, leaving Nix responsible for guest evaluation plus evaluated launch facts.
- [x] Emit the v2 facts manifest from `sandbox-qemu.nix`.
...
flowchart TD
  ...
  H --> I[virtie derives QEMU policy and compiles argv]
